...
export const controlLabels = (mode: GameMode) => mode === "3d"
  ? [["WASD", "前後左右"], ["Space", "ジャンプ"], ["J", "通常攻撃"], ["K", "強攻撃"], ["L", "回避"], ["I", "ガード"]]
  : [["A / D", "移動"], ["W", "ジャンプ"], ["J", "通常攻撃"], ["K", "強攻撃"], ["L", "回避"], ["I", "ガード"]];

export function ControlsDialog({ open, onClose, gameMode }: { open: boolean; onClose: () => void; gameMode: GameMode }) {
  ...
  return <dialog ref={ref} className="help-dialog" aria-labelledby="help-title" onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="dialog-body"><div className="dialog-heading"><Keyboard size={22} /><h2 id="help-title">操作方法</h2><button className="icon-button" aria-label="操作方法を閉じる" onClick={onClose}><X size={20} /></button></div>
      ...
      {gameMode === "3d" && <p>3Dでは W で奥、S で手前へ移動します。相手を自動で向くので、横へ回り込んで攻撃を避けられます。</p>}
      <div className="help-keys">{controlLabels(gameMode).map(([key, label]) => <div key={key}><span>{label}</span><kbd>{key}</kbd></div>)}<div><span>一時停止・再開</span><kbd>Esc</kbd></div></div>
